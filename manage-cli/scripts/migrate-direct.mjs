...
/**
 * Migration script using direct PostgreSQL connection
 */

import fs from 'fs';
import path from 'path';
import { fileURLToPath } from 'url';
import { DirectDatabaseManager } from '../src/lib/direct-db-manager.mjs';
...
class DirectCLIMigrator {
  constructor() {
    this.dbManager = new DirectDatabaseManager();
  }
  ...
  async migrate() {
    ...
    try {
      ...
      const schemaResult = await this.dbManager.ensureSchema();
      ...
      console.log('\n📋 Next steps:');
      console.log('1. Test the new database commands: ./bin/manage.mjs list-db');
      console.log('2. Test installation: ./bin/manage.mjs install-db --all');
      console.log('3. Verify CLI operations work correctly');
...
if (import.meta.url === `file://${process.argv[1]}`) {
  const migrator = new DirectCLIMigrator();
  migrator.migrate().catch(console.error);
}

export { DirectCLIMigrator };